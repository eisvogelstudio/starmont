...
pub const PrefabCache = struct {
    ...
    //pub fn storeMeta(self: *PrefabCache, base: []const u8, meta: no.NodeMeta) void {}
    ...
    //pub fn storeCore(self: *PrefabCache, base: []const u8, meta: co.PrefabDTO) void {}
    ...
    //pub fn storeVisual(self: *PrefabCache, base: []const u8, meta: vi.PrefabDTO) void {}

    fn readMeta(self: *PrefabCache, base: []const u8) !no.NodeMetaDTO {
        var allocator = self.arena.allocator();

        const starmont_path = try std.fmt.allocPrint(allocator, "{s}.ziggy", .{base});
        defer allocator.free(starmont_path);
        const load = try util.ziggy.load(allocator, starmont_path, no.NodeMetaDTO);
    ...
    fn evictLeastUsed(self: *PrefabCache) !void {
        ...
        if (oldest) |key| {
            _ = self.meta.remove(key);
            _ = self.core.remove(key);
            _ = self.visual.remove(key);
            _ = self.last_used.remove(key);
            self.gpa.free(key);
        }
    }
};
